...
class Catalog:
    ...
    def query(self,
        ...
        # validate fileset
        filesets = [f[0] for f in
                    self.db.query(SQL.filesets %
        ...
        # validate timespec
        if timespec:
            self.datetime = datetime.strftime(
                datetime.strptime(timespec, Catalog.datetime_format),
                Catalog.datetime_format)
        else:
            self.datetime = datetime.strftime(datetime.now(),
                                              Catalog.datetime_format)
        ...
        if joblist:
            ...
            self.joblist = ','.join(list(set(
                        [str(int(s.strip())) for s in joblist.split()])))
            self.db.query(SQL.selected_jobs_temp %
                          (self.joblist, self.client_id, self.fileset[1]),
                          fetch=False)
        else:
            ...
            self.db.query(SQL.diff_jobs_temp %
                          (full_jobs[0][1],
                           self.datetime,
                           self.client_id,
                           self.fileset[1]),
            ...
            self.db.query(SQL.incr_jobs_temp %
                          (diff_jobs[-1][1],
                           self.datetime,
                           self.client_id,
                           self.fileset[1]),
        ...
        # get files
        self.files = self.db.query(SQL.files %
                                   (SQL.with_basejobs[self.db.driver] %
                                    (jobs_csl, jobs_csl, jobs_csl, jobs_csl)))
