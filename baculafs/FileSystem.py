...
class FileSystem(Fuse):
    ...
    xattr_fields = ['FileIndex',
                    'JobId',
                    'LStat',
                    'MD5']

    xattr_fields_root = ['client',
                         'fileset',
                         'datetime',
                         'joblist',
                         'cache_prefix']

    xattr_fields_bextract = ['path',
                             'volume',
                             'retries',
                             'state',
                             'pending',
    ...
    def _bacula_stat(self, base64):
        ...
        '''
        st = fuse.Stat()
        lst = dict(zip(FileSystem.bacula_stat_fields,
        ...
        if bs['st_nlink'] > 1 and bs['st_linkfi'] > 0:
            ...
            for file in self.catalog.files:
                if jobid == file[3] and st_linkfi == file[2]:
                    hardlink_target = ('/' if not file[0].startswith('/')
                                       else '') + file[0] + file[1]
    ...
    def _find_volumes(self, path):
        ...
        return (realpath,
                ((symlinkpath, bs.st_atime, bs.st_mtime) if is_symlink
                 else None),
                volumes)
    ...
    def _write_bsr(self, items):
        ...
        '''
        bsrfd, bsrpath = tempfile.mkstemp(suffix='.bsr',
                                          dir=self.cache_bsrpath,
                                          text=True)
